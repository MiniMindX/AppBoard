...
private struct AppBoardCLIIPCRequest: Codable {
    let command: String
    let arguments: [String: String]
}

private struct AppBoardCLIIPCResponse: Codable {
    let ok: Bool
    let output: String?
    let error: String?

    static func success(_ output: String) -> AppBoardCLIIPCResponse {
        AppBoardCLIIPCResponse(ok: true, output: output, error: nil)
    }

    static func failure(_ message: String) -> AppBoardCLIIPCResponse {
        AppBoardCLIIPCResponse(ok: false, output: nil, error: message)
    }
}

enum AppBoardCLIIPCClient {
    static func execute(request: AppBoardCLIRequest, socketPath: String) -> AppBoardCLICommandResult {
        ...
        guard connectSocket(fd: fd, socketPath: socketPath) else {
            return .failure(
                "Please launch AppBoard GUI first.\n" +
                "Run: open -a AppBoard\n" +
                "Make sure \"Command line interface\" is ON in General settings."
            )
        }

        let payload = AppBoardCLIIPCRequest(command: request.command, arguments: request.arguments)
    ...
    private static func sendRequest(_ request: AppBoardCLIIPCRequest, fd: Int32) -> Bool {
    ...
    private static func receiveResponse(fd: Int32) -> AppBoardCLIIPCResponse? {
        guard let lineData = readLine(fd: fd), !lineData.isEmpty else { return nil }
        return try? JSONDecoder().decode(AppBoardCLIIPCResponse.self, from: lineData)
    }
...
final class AppBoardCLIIPCServer {
    typealias CommandHandler = (String, [String: String]) -> AppBoardCLICommandResult

    private let socketPath: String
    private let commandHandler: CommandHandler
    private let queue = DispatchQueue(label: "com.minimindx.appboard.cli.ipc")
    ...
    func start() throws {
        ...
        let fd = socket(AF_UNIX, SOCK_STREAM, 0)
        guard fd >= 0 else {
            throw NSError(domain: "AppBoardCLIIPCServer", code: 1, userInfo: [NSLocalizedDescriptionKey: "Failed to create server socket."])
        }

        if !bindAndListen(fd: fd) {
            close(fd)
            throw NSError(domain: "AppBoardCLIIPCServer", code: 2, userInfo: [NSLocalizedDescriptionKey: "Failed to bind CLI socket."])
        }
    ...
    private func handleClient(_ clientFD: Int32) {
        ...
        guard let line = AppBoardCLIIPCClient.readLine(fd: clientFD),
              let request = try? JSONDecoder().decode(AppBoardCLIIPCRequest.self, from: line) else {
            let response = AppBoardCLIIPCResponse.failure("Invalid CLI request.")
            _ = sendResponse(response, to: clientFD)
            return
        }

        let result = commandHandler(request.command, request.arguments)
        let response: AppBoardCLIIPCResponse
        switch result {
    ...
    private func sendResponse(_ response: AppBoardCLIIPCResponse, to fd: Int32) -> Bool {
        guard let data = try? JSONEncoder().encode(response) else { return false }
        var payload = data
        payload.append(0x0A)
        return AppBoardCLIIPCClient.writeAll(fd: fd, data: payload)
    }
