...
enum AppBoardCLICommandResult {
    case success(String)
    case failure(String)
}

struct AppBoardCLIRequest {
    let command: String
...
struct AppBoardCLIContext {
    let executeRemoteCommand: ((AppBoardCLIRequest) -> AppBoardCLICommandResult)?

    init(executeRemoteCommand: ((AppBoardCLIRequest) -> AppBoardCLICommandResult)? = nil) {
        self.executeRemoteCommand = executeRemoteCommand
    }
}

enum AppBoardRuntimeMode {
    case gui
    case tui
    case cli
}

struct AppBoardCLIArguments {
    let mode: AppBoardRuntimeMode
    let command: String?
    let commandArguments: [String]

    static func parse(_ arguments: [String] = CommandLine.arguments) -> AppBoardCLIArguments {
        ...
        guard let first = args.first else {
            ...
            if isInteractiveTerminalSession() {
                return AppBoardCLIArguments(mode: .tui, command: nil, commandArguments: [])
            }
            return AppBoardCLIArguments(mode: .gui, command: nil, commandArguments: [])
        }
        ...
        case "--cli":
            ...
            return AppBoardCLIArguments(mode: .cli, command: command, commandArguments: commandArgs)
        case "help", "--help", "-h":
            return AppBoardCLIArguments(mode: .cli, command: "help", commandArguments: [])
        default:
            if first.hasPrefix("-") {
                // Unknown flag should not force app into CLI mode.
                return AppBoardCLIArguments(mode: .gui, command: nil, commandArguments: [])
            }
            return AppBoardCLIArguments(mode: .cli, command: first, commandArguments: Array(args.dropFirst()))
        }
    }
...
enum AppBoardCLI {
    ...
    @discardableResult
    static func run(parsed: AppBoardCLIArguments = .parse(), context: AppBoardCLIContext = AppBoardCLIContext()) -> Int32 {
        switch parsed.mode {
        case .gui:
            return 0
        case .tui:
        ...
        case .cli:
            ...
            default:
                fputs("AppBoard CLI: command not implemented yet.\n", stderr)
                printHelp()
                return 1
            }
        }
    ...
    static func printHelp() {
        ...
        Usage:
          ...
          appboard --cli history
          appboard --cli history --limit 20
          appboard --cli example
          appboard --cli search --query "safari"
          appboard --cli search safari
          appboard --cli create-folder --path "/Applications/Mail.app" --path "/Applications/Notes.app" [--name "Utilities"] [--index 12] [--dry-run]
          appboard --cli move --source normal-app --path \"/Applications/Safari.app\" --to normal-index --index 12 [--dry-run]
          appboard --cli move --source normal-app --path \"/Applications/Safari.app\" --to folder-append --target-folder-id <folder-id> [--dry-run]
          appboard --cli move --source folder-app --folder-id <folder-id> --path \"/Applications/Safari.app\" --to folder-index --target-folder-id <folder-id> --index 0 [--dry-run]
          appboard --cli help
          appboard list
          appboard snapshot
          appboard search --query "mail"
          appboard help
        ...
        Requirements:
          - AppBoard GUI must be running for list/snapshot/search/create-folder/move.
          - history/help do not require GUI.
          - "Command line interface" must be ON in General settings.
          - `appboard` (without args) enters TUI only in interactive terminal sessions.
    ...
    @discardableResult
    private static func runTUI(context: AppBoardCLIContext) -> Int32 {
        if isatty(STDIN_FILENO) == 0 {
            fputs("AppBoard TUI requires an interactive terminal.\n", stderr)
            return 1
        }
        ...
        var lineEditor = TUILineEditor(prompt: "appboard> ",
    ...
    @discardableResult
    private static func runRemoteCommand(_ command: String, context: AppBoardCLIContext, printOutput: Bool = true, arguments: [String: String] = [:]) -> Int32 {
        guard let execute = context.executeRemoteCommand else {
            fputs("AppBoard CLI endpoint is unavailable.\n", stderr)
            return 1
        }
        switch execute(AppBoardCLIRequest(command: command, arguments: arguments)) {
    ...
    @discardableResult
    private static func runMove(commandArguments: [String], context: AppBoardCLIContext) -> Int32 {
    ...
    @discardableResult
    private static func runSearch(commandArguments: [String], context: AppBoardCLIContext) -> Int32 {
    ...
    @discardableResult
    private static func runCreateFolder(commandArguments: [String], context: AppBoardCLIContext) -> Int32 {
    ...
    private struct TUILineEditor {
        ...
        mutating func readLine() -> String? {
            ...
            while true {
                ...
                case .clearScreen:
                    AppBoardCLI.clearTerminalScreen()
                    render(buffer: buffer, cursor: cursor)
...
"""#
    ...
    private static func printCLIExamples() {
        print([
            ...
            "  appboard --cli history --limit 20",
            "  appboard --cli search --query \"safari\"",
            "  appboard --cli create-folder --path \"/Applications/Mail.app\" --path \"/Applications/Notes.app\" --name \"Work\"",
            "  appboard --cli move --source normal-app --path \"/Applications/Safari.app\" --to normal-index --index 12 --dry-run"
        ].joined(separator: "\n"))
    }
