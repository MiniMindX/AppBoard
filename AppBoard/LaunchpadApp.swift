...
class AppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate, NSGestureRecognizerDelegate {
    ...
    /// 暴露 launcher 窗口给需要在它消失/丢焦点时保持上下文的调用方（例如删除确认对话框）。
    var launcherWindow: NSWindow? { window }
    /// 调用方在弹出 app-modal 对话框前置为 true，避免 launcher 在 resignKey / resignMain 时被自动收起。
    ...
    private var cliIPCServer: AppBoardCLIIPCServer?
    ...
    private func runHeadlessModeIfRequested() -> Bool {
        let parsed = AppBoardCLIArguments.parse()
        ...
        let context: AppBoardCLIContext
        var socketPathForEndpoint: String?
        if needsEndpoint {
            guard let socketPath = AppBoardCLIIPCConfig.socketPath() else {
                fputs("Failed to resolve AppBoard CLI socket path.\n", stderr)
                isTerminating = true
                NSApp.terminate(nil)
                return true
            }
            socketPathForEndpoint = socketPath
            context = AppBoardCLIContext(executeRemoteCommand: { request in
                AppBoardCLIIPCClient.execute(request: request, socketPath: socketPath)
            })
        } else {
            context = AppBoardCLIContext()
        }
        ...
        if parsed.mode == .tui {
            ...
            DispatchQueue.global(qos: .userInitiated).async { [weak self] in
                _ = AppBoardCLI.run(parsed: parsed, context: context)
                DispatchQueue.main.async { [weak self] in
                    guard let self else { return }
    ...
    private func startCLIEndpointMonitorIfNeeded() {
        ...
        let timer = DispatchSource.makeTimerSource(queue: DispatchQueue(label: "com.minimindx.appboard.cli.endpoint-monitor"))
        var failureCount = 0
        ...
        timer.setEventHandler { [weak self] in
            ...
            let pingResult = AppBoardCLIIPCClient.execute(
                request: AppBoardCLIRequest(command: "ping"),
            ...
            if failureCount >= 2 {
                DispatchQueue.main.async { [weak self] in
                    guard let self else { return }
                    guard self.isHeadlessTUIRuntime, !self.isTerminating else { return }
                    fputs("AppBoard GUI disconnected. Exiting TUI process.\n", stderr)
                    self.isTerminating = true
                    NSApp.terminate(nil)
                }
            }
    ...
    private func requiresCLIEndpoint(_ parsed: AppBoardCLIArguments) -> Bool {
        if parsed.mode == .tui {
            return true
        }
        if parsed.mode == .cli {
    ...
    // delete AppBoard/Gesture/, remove the Gesture settings/AppStore fields,
    ...
    private func updateCLIIPCServer(enabled: Bool) {
        ...
        guard let socketPath = AppBoardCLIIPCConfig.socketPath() else { return }

        if cliIPCServer == nil {
            cliIPCServer = AppBoardCLIIPCServer(socketPath: socketPath) { [weak self] command, arguments in
                guard let self else { return .failure("AppBoard GUI is unavailable.") }
        ...
        do {
            ...
            NSLog("AppBoard: Failed to start CLI IPC server: \(error.localizedDescription)")
        }
    }

    private func handleCLIIPCCommand(_ command: String, arguments: [String: String]) -> AppBoardCLICommandResult {
    ...
    private func handleCLISearch(_ arguments: [String: String]) -> AppBoardCLICommandResult {
    ...
    private func handleCLICreateFolder(_ arguments: [String: String]) -> AppBoardCLICommandResult {
    ...
    private func handleCLIMove(_ arguments: [String: String]) -> AppBoardCLICommandResult {
        if appStore.isLayoutLocked {
    ...
    private func moveNormalAppToNormalIndex(_ arguments: [String: String], dryRun: Bool) -> AppBoardCLICommandResult {
    ...
    private func moveFolderAppToNormalIndex(_ arguments: [String: String], dryRun: Bool) -> AppBoardCLICommandResult {
    ...
    private func moveNormalAppToFolderAppend(_ arguments: [String: String], dryRun: Bool) -> AppBoardCLICommandResult {
        guard let sourcePathRaw = arguments["sourcePath"] else {
            return .failure("Missing sourcePath.")
        }
    ...
    private func moveFolderAppToFolderAppend(_ arguments: [String: String], dryRun: Bool) -> AppBoardCLICommandResult {
        guard let sourceFolderID = arguments["sourceFolderID"], !sourceFolderID.isEmpty else {
            return .failure("Missing sourceFolderID.")
        }
        guard let sourcePathRaw = arguments["sourcePath"] else {
    ...
    private func moveFolderAppToFolderIndex(_ arguments: [String: String], dryRun: Bool) -> AppBoardCLICommandResult {
        guard let sourceFolderID = arguments["sourceFolderID"], !sourceFolderID.isEmpty else {
            return .failure("Missing sourceFolderID.")
        }
        guard let sourcePathRaw = arguments["sourcePath"] else {
    ...
    private func registerGlobalHotKey(_ configuration: AppStore.HotKeyConfiguration) {
        ...
        let status = RegisterEventHotKey(configuration.keyCodeUInt32,
                                         ...
                                         &hotKeyRef)
        if status != noErr {
            NSLog("AppBoard: Failed to register launchpad hotkey (status %d)", status)
            hotKeyRef = nil
        }
    }
    ...
    //                                      &aiHotKeyRef)
    //     if status != noErr {
    //         NSLog("AppBoard: Failed to register AI overlay hotkey (status %d)", status)
    //         aiHotKeyRef = nil
    //     }
    // }
    ...
    private func ensureHotKeyEventHandler() {
        ...
        if status != noErr {
            NSLog("AppBoard: Failed to install hotkey handler (status %d)", status)
        }
    }
