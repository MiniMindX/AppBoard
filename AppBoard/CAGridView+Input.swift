...
extension CAGridView {
    ...
    override func mouseDown(with event: NSEvent) {
        ...
        // Wiggle mode: clicking the top-left close button deletes the app.
        if isWiggleMode, let deleteIndex = deleteBadgeHit(at: location) {
            performDeleteAction(at: deleteIndex)
            return
        }
        ...
        if let (item, index) = itemAt(location) {
            ...
            if event.clickCount == 1 {
                ...
                // 长按计时器：进入 Launchpad 风格的摇动编辑模式。
                // 拖拽仍由 mouseDragged 在移动 > 10px 时启动。
        ...
        if isDraggingItem {
            ...
            if let (item, index) = itemAt(location), index == idx {
                if isBatchSelectionMode {
                    ...
                    // 摇动/编辑模式下：不要启动应用；文件夹仍可打开（便于重命名、拖入应用）。
                    switch item {
                    case .folder:
                        DispatchQueue.main.asyncAfter(deadline: .now() + 0.05) { [weak self] in
                            self?.onItemClicked?(item, index)
                        }
                    case .app, .missingApp, .empty:
                        break
                    }
                } else {
                    // 延迟一点点再触发，让动画效果更明显
