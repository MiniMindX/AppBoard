...
extension CAGridView {
    ...
    func rebuildLayers() {
        ...
        // Re-apply wiggle / delete badges to the freshly built layers.
        if isWiggleMode {
    ...
    func createIconLayer(for item: LaunchpadItem, localIndex: Int, pageIndex: Int) -> CALayer {
        ...
        if case .app = item {
            ...
            // Top-left close button used in wiggle/edit mode (Launchpad-style).
            let deleteBadge = CALayer()
            deleteBadge.name = "deleteBadge"
            deleteBadge.isHidden = true
            deleteBadge.backgroundColor = NSColor(calibratedWhite: 0.18, alpha: 0.95).cgColor
            deleteBadge.borderColor = NSColor.white.withAlphaComponent(0.85).cgColor
            deleteBadge.borderWidth = 1.0
            deleteBadge.zPosition = 30
            deleteBadge.contentsScale = NSScreen.main?.backingScaleFactor ?? 2.0
            deleteBadge.shadowColor = NSColor.black.cgColor
            deleteBadge.shadowOffset = CGSize(width: 0, height: -1)
            deleteBadge.shadowRadius = 2
            deleteBadge.shadowOpacity = 0.35

            let cross = CAShapeLayer()
            cross.name = "deleteBadgeCross"
            cross.fillColor = NSColor.clear.cgColor
            cross.strokeColor = NSColor.white.cgColor
            cross.lineCap = .round
            cross.lineJoin = .round
            cross.contentsScale = NSScreen.main?.backingScaleFactor ?? 2.0
            deleteBadge.addSublayer(cross)
            containerLayer.addSublayer(deleteBadge)
        }
        ...
        for (pageIndex, pageLayers) in iconLayers.enumerated() {
            for (localIndex, containerLayer) in pageLayers.enumerated() {
                ...
                if let iconLayer = containerLayer.sublayers?.first(where: { $0.name == "icon" }) {
                    ...
                    if let badgeLayer = containerLayer.sublayers?.first(where: { $0.name == "deleteBadge" }) {
                        let badgeSize = max(18, min(26, actualIconSize * 0.30))
                        // 徽章整体落在图标包围盒内、贴住可见圆角弧线：
                        // 系统图标通常自带 ~6-10% 的透明边距，向内偏移到弧线上才会"贴住"。
                        let cornerInset = max(2.0, actualIconSize * 0.05)
                        let badgeX = iconFrame.minX + cornerInset
                        let badgeY = iconFrame.maxY - badgeSize - cornerInset
                        badgeLayer.frame = CGRect(x: badgeX, y: badgeY, width: badgeSize, height: badgeSize)
                        badgeLayer.cornerRadius = badgeSize * 0.5
                        if let crossLayer = badgeLayer.sublayers?.first(where: { $0.name == "deleteBadgeCross" }) as? CAShapeLayer {
                            crossLayer.frame = badgeLayer.bounds
                            crossLayer.lineWidth = max(1.6, badgeSize * 0.12)
                            crossLayer.path = CAGridView.deleteBadgeCrossPath(in: badgeLayer.bounds)
                        }
                    }
                }
