...
final class AppStore: ObservableObject {
    ...
    // Safe to remove together with AppBoard/Gesture/ and gesture UI wiring
    ...
    private static let cliShimMarker = "# AppBoard CLI shim"
    private static let cliPathSnippetHeader = "# >>> AppBoard CLI >>>"
    private static let cliPathSnippetFooter = "# <<< AppBoard CLI <<<"
    ...
    fileprivate static let updateNotificationCategoryIdentifier = "appboard.update.category"
    fileprivate static let updateNotificationDownloadActionIdentifier = "appboard.update.download"
    ...
    @Published var isStartOnLogin: Bool = {
        ...
        didSet {
            ...
            do {
                ...
                NSLog("AppBoard: Failed to update login item setting - %@", error.localizedDescription)
    ...
    private func cliCommandTargets() -> [String] {
        // 仅在用户私有目录写 CLI shim：
        //   - ~/.local/bin/   通用 Unix 用户级目录（XDG）
        //   - ~/bin/          很多 zsh/bash 默认会加进 PATH
        // 故意不再写 /opt/homebrew/bin 和 /usr/local/bin，避免污染 Homebrew 管辖区。
        let homePath = FileManager.default.homeDirectoryForCurrentUser.path
        return [
            "\(homePath)/.local/bin/appboard",
            "\(homePath)/bin/appboard"
        ]
    }
    ...
    private func installCLIShim(at shimPath: String, executablePath: String) -> Bool {
        ...
        Usage:
          ...
          appboard --cli help
          appboard --cli list
          appboard --cli snapshot
          appboard --cli search --query "safari"
          appboard --cli move --source normal-app --path "/Applications/Thaw.app" --to folder-append --target-folder-id <folder-id>

        Notes:
          - Keep `--cli --help` and `--cli help` for full in-app CLI help.
          - AppBoard GUI must be running for list/snapshot/search/move.
    ...
    private func uninstallCLIShim(at shimPath: String) -> Bool {
        ...
        let isManagedShim: Bool = {
            ...
            if let destination = try? fileManager.destinationOfSymbolicLink(atPath: shimPath),
               destination.contains("/AppBoard.app/Contents/MacOS/AppBoard") {
                return true
            }
            return false
        }()
    ...
    func loadAllOrder() {
        guard let modelContext else {
            print("AppBoard: ModelContext is nil, cannot load persisted order")
            return
        }
        
        print("AppBoard: Attempting to load persisted order data...")
        
        // 优先尝试从新的"页-槽位"模型读取
        if loadOrderFromPageEntries(using: modelContext) {
            print("AppBoard: Successfully loaded order from PageEntryData")
            return
        }
        
        print("AppBoard: PageEntryData not found, trying legacy TopItemData...")
        // 回退：旧版全局顺序模型
        loadOrderFromLegacyTopItems(using: modelContext)
        print("AppBoard: Finished loading order from legacy data")
    }
    ...
    func saveAllOrder() {
        guard let modelContext else {
            print("AppBoard: ModelContext is nil, cannot save order")
            return
        }
        guard !items.isEmpty else {
            print("AppBoard: Items list is empty, skipping save")
            return
        }

        print("AppBoard: Saving order data for \(items.count) items...")
        
        // 写入新模型：按页-槽位
        do {
            let existing = try modelContext.fetch(FetchDescriptor<PageEntryData>())
            print("AppBoard: Found \(existing.count) existing entries, clearing...")
            ...
            print("AppBoard: Successfully saved order data")
            ...
            print("AppBoard: Error saving order data: \(error)")
        }
    }
    ...
    /// 手动刷新（模拟全新启动的完整流程）
    func refresh() {
        print("AppBoard: Manual refresh triggered")
    ...
    /// Launchpad-style "X" delete: prompt the user, then move the .app bundle to the Trash.
    /// If the user has configured an external uninstall tool we hand off to that instead.
    /// 优先以 sheet 形式挂在 launcher 窗口上，让 launcher 保持可见（仿旧 Launchpad 体验）。
    func deleteAppFromGrid(_ app: AppInfo) {
        if uninstallToolAppURL != nil {
            _ = openConfiguredUninstallTool(for: app)
            return
        }
        ...
        // Refuse to delete system-protected apps even if the caller asks.
        if target.path.hasPrefix("/System/") || target.path.hasPrefix("/Library/Apple/") {
            NSSound.beep()
            return
        }
        ...
        // 把目标 App 的图标放进 alert，匹配旧 Launchpad 删除确认窗的视觉。
        alert.icon = IconStore.shared.icon(forPath: target.path)
        ...
        case .simplifiedChinese:
            titleFormat = "确定要删除应用程序 \"%@\" 吗？"
            deleteButton = "删除"
            cancelButton = "取消"
        case .traditionalChinese:
            titleFormat = "確定要刪除應用程式 \"%@\" 嗎？"
            deleteButton = "刪除"
            cancelButton = "取消"
        case .japanese:
            titleFormat = "アプリケーション \"%@\" を削除してもよろしいですか？"
            deleteButton = "削除"
            cancelButton = "キャンセル"
        default:
            titleFormat = "Are you sure you want to delete the application \"%@\"?"
            deleteButton = "Delete"
            cancelButton = "Cancel"
        }
        alert.messageText = String(format: titleFormat, app.name)
        alert.addButton(withTitle: deleteButton)   // .alertFirstButtonReturn，右侧默认按钮（蓝）
        alert.addButton(withTitle: cancelButton)   // .alertSecondButtonReturn

        // 注意：launcher 是 borderless + .floating + .transient 窗口，与 beginSheetModal(for:)
        // 不兼容（会触发 ViewBridge RemoteViewService 终止并把窗口搞坏，导致 AppBoard 再也起不来）。
        // 这里改用 app-modal runModal，配合 AppDelegate.suppressAutoHide 阻止 launcher
        // 在 resignKey/resignMain 时被 autoHideIfNeeded 收起；NSAlert 默认 .modalPanel(8) 高于
        // launcher 的 .floating(3)，会自然叠在 launcher 之上。
        let delegate = AppDelegate.shared
        delegate?.suppressAutoHide = true
        defer {
            delegate?.suppressAutoHide = false
            // alert 结束后，让 launcher 重新成为 key window，恢复键盘/鼠标交互。
    ...
    private func applyCurrentAppIcon() {
        ...
        DispatchQueue.main.async {
            ...
            if success {
                ...
                NSLog("AppBoard: Failed to update application bundle icon at %@", bundlePath)
            }
        }
    }
    ...
    private func fetchLatestRelease() async throws -> GitHubRelease {
        let url = URL(string: "https://api.github.com/repos/MiniMindX/AppBoard/releases/latest")!
    ...
    func sendTestUpdateNotification() {
        enqueueUpdateNotification(
            ...
            releaseURL: URL(string: "https://github.com/MiniMindX/AppBoard/releases")
        )
    }
